...
if __name__ =='__main__':
    caffe.set_mode_gpu()
    net = caffe.Net('./c3d_ucf101_test.prototxt','/scratch/xf362/c3d_ucf101_siamese/test_new_57000.000000',caffe.TEST)
    iteration = 1000
    pos = []
    neg = []
    testRes = {}
    for i in range(iteration):
      data = net.forward()
      data1 = net.blobs['prob'].data
      data2 = net.blobs['prob_p'].data
      for j in range(5):	
        if data['label'][j,:] == 1:
           dis_pos = scipy_stats.entropy(data1[j,:],data2[j,:])
           pos.append(dis_pos)
        else:
           dis_neg = scipy_stats.entropy(data1[j,:],data2[j,:])
           neg.append(dis_neg)
      print i 
    testRes['pos'] = pos
    testRes['neg'] = neg
    scipy_io.savemat(open('testResult_prob.mat','wb'), testRes)
